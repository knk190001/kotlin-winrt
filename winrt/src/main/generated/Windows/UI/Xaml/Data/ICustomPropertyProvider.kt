...
public interface ICustomPropertyProvider : NativeMapped, IWinRTInterface {
  ...
  public interface WithDefault : ICustomPropertyProvider {
    public val __1567289503_Ptr: Pointer?

    public val _1567289503_VtblPtr: Pointer?
      get() = __1567289503_Ptr?.getPointer(0)

    @InterfaceMethod(0)
    public override fun GetCustomProperty(name: String?): ICustomProperty? {
      val fnPtr = _1567289503_VtblPtr!!.getPointer(6L * POINTER_SIZE)
      val fn = Function.getFunction(fnPtr, ALT_CONVENTION)
      val result = makeByReferenceType<ICustomProperty>()
      val hr = fn.invokeHR(arrayOf(__1567289503_Ptr, marshalToNative(name), result))
      if (hr.toInt() != 0) {
    ...
    public override fun GetIndexedProperty(name: String?, type: TypeName?): ICustomProperty? {
      val fnPtr = _1567289503_VtblPtr!!.getPointer(7L * POINTER_SIZE)
      val fn = Function.getFunction(fnPtr, ALT_CONVENTION)
      val result = makeByReferenceType<ICustomProperty>()
      val hr = fn.invokeHR(arrayOf(__1567289503_Ptr, marshalToNative(name), marshalToNative(type),
          result))
      if (hr.toInt() != 0) {
        throw RuntimeException(hr.toString())
      }
    ...
    public override fun GetStringRepresentation(): String? {
      val fnPtr = _1567289503_VtblPtr!!.getPointer(8L * POINTER_SIZE)
      val fn = Function.getFunction(fnPtr, ALT_CONVENTION)
      val result = makeByReferenceType<String>()
      val hr = fn.invokeHR(arrayOf(__1567289503_Ptr,  result))
      if (hr.toInt() != 0) {
        throw RuntimeException(hr.toString())
      }
    ...
    public override fun get_Type(): TypeName? {
      val fnPtr = _1567289503_VtblPtr!!.getPointer(9L * POINTER_SIZE)
      val fn = Function.getFunction(fnPtr, ALT_CONVENTION)
  ...
  public class ICustomPropertyProvider_Impl(
    ...
    public override val __1567289503_Ptr: Pointer?
      get() = pointer
  }
  ...
  public object ABI : IABI<ICustomPropertyProvider, MemoryAddress> {
    ...
    public override fun toNative(obj: ICustomPropertyProvider): MemoryAddress =
        MemoryAddress.ofLong(Pointer.nativeValue((obj as WithDefault).__1567289503_Ptr))
  }
