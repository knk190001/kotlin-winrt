...
public fun interface AsyncActionWithProgressCompletedHandlerBody<TProgress> {
  public operator fun invoke(asyncInfo: IAsyncActionWithProgress<TProgress>?,
      asyncStatus: AsyncStatus?): Unit
}

@ABIMarker(AsyncActionWithProgressCompletedHandler.ABI::class)
@GenericType
@DynamicSignature
@Guid("9c029f91cc8444fdac260a6c4e555281")
@WinRTByReference(AsyncActionWithProgressCompletedHandler.ByReference::class)
public class AsyncActionWithProgressCompletedHandler<TProgress>(
  public val type: KType? = null,
  public val body: AsyncActionWithProgressCompletedHandlerBody<TProgress>? = null,
  ...
  public companion object {
    public val nativeFnHandle: MethodHandle
      ...
      get() = MethodHandles.lookup().findStatic(
          AsyncActionWithProgressCompletedHandler::class.java, "nativeFn",
        MethodType.methodType(Int::class.java, 
        KType::class.java, AsyncActionWithProgressCompletedHandlerBody::class.java,
            MemoryAddress::class.java, MemoryAddress::class.java, Int::class.java, )

      )

    public inline operator fun <reified TProgress>
        invoke(fn: AsyncActionWithProgressCompletedHandlerBody<TProgress>):
        AsyncActionWithProgressCompletedHandler<TProgress> {
      val session = MemorySession.global()
      val handleDescriptorPair =
          transformParameterizedHandle(typeOf<AsyncActionWithProgressCompletedHandler<TProgress>>())
      ...
      val newDelegate =
          AsyncActionWithProgressCompletedHandler(typeOf<AsyncActionWithProgressCompletedHandler<TProgress>>(),
          fn, Memory(12))
      val guid = guidOf<AsyncActionWithProgressCompletedHandler<TProgress>>()
    ...
    public operator fun <TProgress> invoke(type: KType, ptr: Pointer?):
      ...
      val body: AsyncActionWithProgressCompletedHandlerBody<TProgress> =
          AsyncActionWithProgressCompletedHandlerBody<TProgress>  {
      ...
      return AsyncActionWithProgressCompletedHandler(type, body, thisPtr)
    }

    public inline operator fun <reified TProgress> invoke(pointer: Pointer?):
        AsyncActionWithProgressCompletedHandler<TProgress> {
      val type = typeOf<AsyncActionWithProgressCompletedHandler<TProgress>>()
      return AsyncActionWithProgressCompletedHandler(type, pointer)
    }

    @JvmStatic
    public fun <TProgress> nativeFn(
      type: KType,
      fn: AsyncActionWithProgressCompletedHandlerBody<TProgress>,
  ...
  public class ByReference<TProgress> : com.sun.jna.ptr.ByReference(POINTER_SIZE),
      IByReference<AsyncActionWithProgressCompletedHandler<TProgress>>, ISpecializable {
    ...
    public override fun getValue(): AsyncActionWithProgressCompletedHandler<TProgress> =
        AsyncActionWithProgressCompletedHandler<TProgress>(type!!, pointer.getPointer(0))

    public fun setValue(value: AsyncActionWithProgressCompletedHandler<TProgress>): Unit {
      pointer = value.pointer!!
    }
  }

  public object ABI : IParameterizedABI<AsyncActionWithProgressCompletedHandler<*>, MemoryAddress> {
    public override val layout: MemoryLayout = ValueLayout.ADDRESS

    public override fun fromNative(type: KType, segment: MemoryAddress):
        AsyncActionWithProgressCompletedHandler<*> {
      val address = segment.toRawLongValue()
      return AsyncActionWithProgressCompletedHandler<Unit>(type, Pointer(address))
    }

    public override fun toNative(obj: AsyncActionWithProgressCompletedHandler<*>): MemoryAddress =
